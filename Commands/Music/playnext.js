    ...
    rethink.table('guildConfig')
        ...
        .run(connection, (err, result) => {
            ...
            callback();
                ...
                const message = await msg.channel.send('trying to add your Song/Playlist to the queue as next Song....')
                music.addAsNextToQueue(Link, msg.member)
                .then(result => {
                    message.edit(result[0])
                    music.playqueue(msg.guild, msg.channel)
                })
                .catch(err => {
                    message.edit(`Could not add the Song/Playlist as next Song with following reason: ${err}`)
                })
            })
        })
}
...
exports.help = {
    'name': 'playnext',
    'description': 'add a song as the next song that will be playing',
    'usage': 'playnext [Songname/Youtube URL]'
}
