...
class WeatherCommand extends Commands {
	...
	run(msg, params) {
		...
		weather.find({ search: params.join(' '), degreeType: 'C' }, async (err, result) => {
			if (err) return msg.channel.send('Something went wrong! did you spell the city name right?');
			...
			try {
				base.src = await this.readFileAsync('./materials/pictures/weather.png');
				humidity.src = await this.readFileAsync('./materials/icons/humidity.png');
				windspeed.src = await this.readFileAsync('./materials/icons/wind.png');
			} catch (error) {
