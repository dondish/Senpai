...
module.exports = class PostgreSQL extends Provider {
	...
	async init() {
		const connection = util.mergeDefault({
			host: this.client.databaseConfig.databaseHost,
			port: this.client.databaseConfig.port,
			db: this.client.databaseConfig.databaseHost,
			user: this.client.databaseConfig.databaseUser,
			password: this.client.databaseConfig.databasePW,
			options: {
				max: 20,
